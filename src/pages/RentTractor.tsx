...
const RentTractor = () => {
  ...
  const fetchTractor = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/tractors/${id}`);
  ...
  const handleConfirmRent = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/tractors/confirm-rental`, {
        method: 'POST',
        headers: {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="relative rounded-xl overflow-hidden bg-gradient-to-br from-primary/20 to-secondary/20 aspect-video flex items-center justify-center">
            ...
            {tractor && (
              <span
                className={`absolute top-4 right-4 status-badge ${tractor.isAvailable ? 'status-badge-available' : 'status-badge-rented'
                  }`}
              >
                <span
                  className={`h-2 w-2 rounded-full ${tractor.isAvailable ? 'bg-success' : 'bg-danger'
                    }`}
                />
          ...
          <div className="space-y-6">
            <div className="bg-card rounded-xl border border-border p-6 sticky top-24">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-4 rounded-lg border text-center transition-all ${rentalType === 'hourly'
                      ? 'border-primary bg-primary/10'
                      : 'border-border hover:border-primary/50'
                      }`}
                  >
                    <Clock className={`h-5 w-5 mx-auto mb-2 ${rentalType === 'hourly' ? 'text-primary' : 'text-muted-foreground'
                      }`} />
                    <p className={`font-semibold ${rentalType === 'hourly' ? 'text-primary' : 'text-foreground'
                      }`}>
                      Hourly
                    </p>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-lg border text-center transition-all ${rentalType === 'daily'
                      ? 'border-primary bg-primary/10'
                      : 'border-border hover:border-primary/50'
                      }`}
                  >
                    <Calendar className={`h-5 w-5 mx-auto mb-2 ${rentalType === 'daily' ? 'text-primary' : 'text-muted-foreground'
                      }`} />
                    <p className={`font-semibold ${rentalType === 'daily' ? 'text-primary' : 'text-foreground'
                      }`}>
                      Daily
                    </p>
                    <div className="flex items-center justify-center text-sm text-muted-foreground">
              ...
              <button
                ...
                className={`w-full py-4 rounded-lg font-semibold transition-all ${tractor.isAvailable
                  ? 'btn-primary'
